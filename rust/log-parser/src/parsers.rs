...
fn splitn(separator: char) -> impl Parser<String> {
    generalize_lifetime(move |s| {
        let result: Vec<&str> = s.splitn(2, separator.to_owned()).collect();
        if result.len() == 2 {
            Some((result[0].to_owned(), result[1]))
        } else {
...
fn test_splitn() {
    let parser = splitn(' ');
    assert_eq!(Some(("123".to_owned(), "456")), parser("123 456"));
    assert_eq!(Some(("123456".to_owned(), "")), parser("123456"));
}

fn skip(separator: char) -> impl Parser<()> {
    map(splitn(separator), |_s| ())
}

#[test]
fn test_skip() {
    let parser = skip(' ');
    assert_eq!(Some(((), "456")), parser("123 456"));
}

fn paren(start: char, end: char) -> impl Parser<((), String)> {
    join!(skip(start), splitn(end))
}

#[test]
fn test_paren() {
    let parser = paren('(', ')');
    assert_eq!(Some((((), "3 4".into()), "56")), parser("12(3 4)56"));
}
